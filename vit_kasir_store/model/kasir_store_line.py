...
class KasirStoreLine(models.Model):

    _name = "vit.kasir_store_line"
    _description = "vit.kasir_store_line"
    ...
    # location_id     = fields.Many2one('stock.location', related='swap_id.location_id', string='Last Location', store=True)
    # is_complete     = fields.Boolean('Is Complete', compute="_is_comp")
    ...
    # @api.depends('pengukuran_karyawan_target')
    # def _is_comp(self):
    #     for ksl in self:
    #         if ksl.pengukuran_karyawan_target:
    #             data_ukur = self.env["vit.data_pengukuran"].search([('karyawan_id','=',ksl.pengukuran_karyawan_target.id)])
    #             line_ids = self.env["vit.kasir_store_line"].search([('store_id','=',ksl.store_id.id),('data_pengukuran_id_target','=',ksl.data_pengukuran_id_target.id)])
    #             full_qty = sum(du.qty for du in data_ukur)
    #             count_packing = len(set(line_ids.ids))
    #             if count_packing == full_qty:
    #                 ksl.is_complete = True

    def check_data_double(self, lot_ids):
        lots = str(tuple(lot_ids.ids)).replace(",)",")")
        sql = "select ks.id from vit_kasir_store_line ks left join stock_production_lot lot on ks.lot_id_target = lot.id where lot.id in %s and ks.store_id != %s and lot.store is true limit 1" % (lots, self.store_id.id)
    ...
    def swap_barcode(self):
        ...
        self.env.cr.execute("""
            UPDATE stock_production_lot SET nik = '%s', karyawan = '%s', nama_bordir = '%s', pengukuran_karyawan = %s,
                    data_pengukuran_id = '%s', polybag = '%s', divisi_id = %s, lokasi_id = %s, jabatan_id = %s, lot = %s
            WHERE id = '%s'
            """ % (self.nik_target, karyawan_target, bordir_target, self.pengukuran_karyawan_target.id, self.data_pengukuran_id_target.id,
                   self.polybag_target, divisi_target, lokasi_target, jabatan_target, lot_target, self.lot_id_sumber.id))
        ...
        self.env.cr.execute("""
            ...
            """ % (self.nik_sumber, karyawan_sumber, bordir_sumber, self.pengukuran_karyawan_sumber.id, self.data_pengukuran_id_sumber.id,
                   self.polybag_sumber, divisi_sumber, lokasi_sumber, jabatan_sumber, lot_sumber, self.lot_id_target.id))
    ...
    #     for rec in self:
    #         if rec.divisi_target:
    #             divisi_target = rec.divisi_target.id
    #         else:
    #             divisi_target = 'Null'

    #         if rec.lokasi_target:
    #             lokasi_target = rec.lokasi_target.id
    #         else:
    #             lokasi_target = 'Null'

    #         if rec.jabatan_target:
    #             jabatan_target = rec.jabatan_target.id
    #         else:
    #             jabatan_target = 'Null'

    #         if rec.location_id_target:
    #             location_id_target = rec.location_id_target.id
    #         else:
    #             location_id_target = 'Null'

    #         if rec.divisi_sumber.id: 
    #             divisi_sumber = rec.divisi_sumber.id            
    #         else:
    #             divisi_sumber = 'Null'

    #         if rec.lokasi_sumber:
    #             lokasi_sumber = rec.lokasi_sumber.id
    #         else:
    #             lokasi_sumber = 'Null'

    #         if rec.jabatan_sumber:
    #             jabatan_sumber = rec.jabatan_sumber.id
    #         else:
    #             jabatan_sumber = 'Null'

    #         if rec.location_id_sumber:
    #             location_id_sumber = rec.location_id_sumber.id
    #         else:                
    #             location_id_sumber = 'Null'

    #         if rec.lot_id_sumber:
    #             # update lot data tujuan
    #             self.env.cr.execute("""
    #                 UPDATE stock_production_lot SET nik = '%s', karyawan = '%s', nama_bordir = '%s', pengukuran_karyawan = %s,
    #                         data_pengukuran_id = '%s', polybag = '%s', divisi_id = %s, lokasi_id = %s, jabatan_id = %s, location_id = %s
    #                 WHERE id = '%s'
    #                 """ % (rec.nik_target, rec.karyawan_target, rec.bordir_target, rec.pengukuran_karyawan_target.id, rec.data_pengukuran_id_target.id,
    #                        rec.polybag_target, divisi_target, lokasi_target, jabatan_target, location_id_target, rec.lot_id_sumber.id))
    #             # update lot data asal
    #             self.env.cr.execute("""
    #                 UPDATE stock_production_lot SET nik = '%s', karyawan = '%s', nama_bordir = '%s', pengukuran_karyawan = %s,
    #                         data_pengukuran_id = '%s', polybag = '%s', divisi_id = %s, lokasi_id = %s, jabatan_id = %s, location_id = %s
    #                 WHERE id = '%s'
    #                 """ % (rec.nik_sumber, rec.karyawan_sumber, rec.bordir_sumber, rec.pengukuran_karyawan_sumber.id, rec.data_pengukuran_id_sumber.id,
    #                        rec.polybag_sumber, divisi_sumber, lokasi_sumber, jabatan_sumber, location_id_sumber, rec.lot_id_target.id))
                
    #             # create workorder history lot tujuan
    #             self.env.cr.execute("""
    #                 INSERT INTO swap_history_barcode (id, karyawan, nik, swap_lot_id, store_doc, lot_id, date)
    #                 VALUES (nextval('swap_history_barcode_id_seq'), '%s', '%s', %s, %s, %s, (now() at time zone 'UTC'))
    #                 """ % (rec.karyawan_target, rec.nik_target, rec.lot_id_target.id, rec.store_id.id, rec.lot_id_sumber.id))
    #             # create workorder history lot asal
    #             self.env.cr.execute("""
    #                 INSERT INTO swap_history_barcode (id, karyawan, nik, swap_lot_id, store_doc, lot_id, date)
    #                 VALUES (nextval('swap_history_barcode_id_seq'), '%s', '%s', %s, %s, %s, (now() at time zone 'UTC'))
    #                 """ % (rec.karyawan_sumber, rec.nik_sumber, rec.lot_id_sumber.id, rec.store_id.id, rec.lot_id_target.id))

    #             rec.update({
    #                         'nik_target' : rec.lot_id_target.nik,
    #                         'karyawan_target' : rec.lot_id_target.karyawan,
    #                         'bordir_target' : rec.lot_id_target.nama_bordir,
    #                         'pengukuran_karyawan_target' : rec.lot_id_target.pengukuran_karyawan.id,
    #                         'data_pengukuran_id_target' : rec.lot_id_target.data_pengukuran_id.id,
    #                         'polybag_target' : rec.lot_id_target.polybag,
    #                         'divisi_target' : rec.lot_id_target.divisi_id.id,
    #                         'lokasi_target' : rec.lot_id_target.lokasi_id.id,
    #                         'jabatan_target': rec.lot_id_target.jabatan_id.id,
    #                         'location_id_target' : rec.lot_id_sumber.location_id.id,
    #                         'nik_sumber' : rec.lot_id_sumber.nik,
    #                         'karyawan_sumber' : rec.lot_id_sumber.karyawan,
    #                         'bordir_sumber' : rec.lot_id_sumber.nama_bordir,
    #                         'pengukuran_karyawan_sumber' : rec.lot_id_sumber.pengukuran_karyawan.id,
    #                         'data_pengukuran_id_sumber' : rec.lot_id_sumber.data_pengukuran_id.id,
    #                         'polybag_sumber' : rec.lot_id_sumber.polybag,
    #                         'divisi_sumber' : rec.lot_id_sumber.divisi_id.id,
    #                         'lokasi_sumber' : rec.lot_id_sumber.lokasi_id.id,
    #                         'jabatan_sumber': rec.lot_id_sumber.jabatan_id.id,
    #                         'location_id_sumber' : rec.lot_id_sumber.location_id.id,
    #                         'is_swap' : False,
    #                         })
