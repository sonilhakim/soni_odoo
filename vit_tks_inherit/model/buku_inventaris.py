...
class buku_inventaris(models.Model):
    ...
    _inherit = "vit.buku_inventaris"

    date  = fields.Date(string="Tanggal Pembukuan", required=False, default=lambda self:time.strftime("%Y-%m-%d"))
    status = fields.Selection(selection=[('draft', 'Draft'), ('confirm', 'Confirm'), ('open', 'Running'), ('close', 'Close')],  string="Status",  readonly=True, states={"draft" : [("readonly",False)]},  help="")

    @api.model
    def create(self, vals):
        if not vals.get("name", False) or vals["name"] == "New":
            vals["name"] = self.env["ir.sequence"].next_by_code("vit.buku_inventaris") or "Error Number!!!"
        return super(buku_inventaris, self).create(vals)
    ...
    def action_reload(self, ):
        sql = "delete from vit_buku_inventaris_detail where buku_inventaris_id = %s"
        self.env.cr.execute(sql, (self.id,))
        sql = """
                insert into vit_buku_inventaris_detail (name, type, tahun_perolehan, jumlah, satuan, kondisi, asal, buku_inventaris_id)
                select ast.name, ast.tipe_barang, ast.date, ast.qty, ast.satuan_id, ast.condition, ast.asal, %s
        ...
        if self.kategori_id:
            sql += ' and cat.id = %s ' %self.kategori_id.id
        if self.lokasi_id:
            sql += ' and loc.id = %s ' %self.lokasi_id.id
