...
class mutasi_status_pegawai(models.Model):
    ...
    _inherit = "vit.mutasi_status_pegawai"
    ...
    def create(self, vals):
        if not vals.get("name", False) or vals["name"] == "New":
            vals["name"] = self.env["ir.sequence"].next_by_code("vit.mutasi_status_pegawai") or "Error Number!!!"
        return super(mutasi_status_pegawai, self).create(vals)

    def action_confirm(self):
        self.state = STATES[1][0]

    def action_done(self):
        for d in self.status_detail_ids:
            self.env.cr.execute("update hr_employee set status_pegawai=%s where id = %s",( d.status_tujuan.id, d.employee_id.id ))
