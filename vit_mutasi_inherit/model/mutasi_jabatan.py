...
class mutasi_jabatan(models.Model):
    ...
    def create(self, vals):
        if not vals.get("name", False) or vals["name"] == "New":
            vals["name"] = self.env["ir.sequence"].next_by_code("vit.mutasi_jabatan") or "Error Number!!!"
        return super(mutasi_jabatan, self).create(vals)
    ...
    def action_done(self):
        ...
        for d in self.mutasi_jab_ids:
            self.env.cr.execute("update hr_employee set jabatanf_id=%s where id = %s",( d.jabatan_tujuan.id, d.employee_id.id ))
