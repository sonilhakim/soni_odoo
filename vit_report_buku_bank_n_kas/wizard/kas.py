...
class export_buku_kas_wizard(models.TransientModel):
    _name = 'vit.export_buku_kas'
    ...
    def get_datas(self):
        ...
        if self.journal_id:
            ...
            sql += " AND aj.type = 'cash' GROUP BY aml.id, am.name, aa.id ORDER BY aml.date "
        ...
        res['Report Buku Kas'] = [headers, initial, values, final]        
        return res
    ...
    def confirm_button(self):
        ...
        for sheet, header_content in datas.items():
            ...
            row = 0
            if self.journal_id:
                worksheet.write(row, column, 'Buku Kas '+self.journal_id.name, cell_format['info'])
            else:
                worksheet.write(row, column, 'Buku Kas', cell_format['info'])
            row = 1
        ...
        if self.journal_id:
            filename = 'Report Buku Kas ' + self.journal_id.name + ' ' + bulan + ' ' + self.year + '.xlsx'
        else:
            filename = 'Report Buku Kas ' + ' ' + self.year + '.xlsx'
        self.write({'export_file':result, 'export_filename':filename})
        return {
            'name': "Export Complete, total %s records" % i,
            'type': 'ir.actions.act_window',
            'res_model': 'vit.export_buku_kas',
            'view_mode': 'form',
            'view_type': 'form',
            'res_id': self.id,
            'views': [(False, 'form')],
