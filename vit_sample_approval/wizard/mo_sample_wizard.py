...
class MOsampleWizard(models.TransientModel):
    _name = "mo.sample.wizard"

    @api.model
    def _get_default_picking_type(self):
        # import pdb;pdb.set_trace()
        if self._context.get('active_id'):
            req_obj = self.env['vit.sample_approval_line']
            req_det = req_obj.browse(self._context.get('active_id'))
            company_id = req_det.sample_approval_id.company_id        
    ...
    def create_mo_sample(self):
        ...
        if req_detail_id:
            ...
            product_tmpl_id = req.product_sample_id
            ...
            datas = {
                ...
                # 'routing_id'                : req.routing_id.id,
                'origin'                    : req.sample_approval_id.name,
                'sample'                    : True,
                'approval_line_id'          : req.id,
                'product_qty'               : qty,
                'picking_type_id'           : self.picking_type_id.id,
                'location_src_id'           : self.location_src_id.id,
                'location_dest_id'          : self.location_dest_id.id,
                'date_planned_start'        : self.date_planned,
                'company_id'                : req.sample_approval_id.company_id.id,
                'partner_id'                : req.sample_approval_id.partner_id.id
            }
