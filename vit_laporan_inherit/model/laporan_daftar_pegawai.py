...
class laporan_daftar_pegawai(models.Model):
    ...
    _inherit = "vit.laporan_daftar_pegawai"

    @api.model
    def create(self, vals):
        if not vals.get("name", False) or vals["name"] == "New":
            vals["name"] = self.env["ir.sequence"].next_by_code("vit.laporan_daftar_pegawai") or "Error Number!!!"
        return super(laporan_daftar_pegawai, self).create(vals)
    ...
    def action_reload(self, ):
        sql = "delete from vit_laporan_detail where lap_daftar_pegawai_id = %s"
        self.env.cr.execute(sql, (self.id,))
        sql = """
                insert into vit_laporan_detail (name, nip, golongan, jabatan, lap_daftar_pegawai_id)
