...
class EFakturSend(models.TransientModel):
    _name = 'e_faktur.send'
    _inherits = {'mail.compose.message':'composer_id'}
    _description = 'EFaktur Send'
    ...
    invoice_ids = fields.Many2many('account.invoice', 'account_invoice_efaktur_send_rel', string='Invoices')
    ...
    def default_get(self, fields):
        res = super(EFakturSend, self).default_get(fields)
            ...
            if wizard.is_email and len(wizard.invoice_ids) > 1:
                ...
                if invoices:
                    wizard.invoice_without_email = "%s\n%s" % (
                        _("The following E-Faktur(s) will not be sent by email, because the customers don't have email address."),
    ...
    def _send_email(self):
        if self.is_email:
            ...
            if self.env.context.get('mark_efaktur_as_sent'):
                self.mapped('invoice_ids').write({'efaktur_sent': True})
